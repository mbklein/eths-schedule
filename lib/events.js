...
function buildSchedule(calendar, schedule) {
  let result = [
    "BEGIN:VCALENDAR",
    "METHOD:PUBLISH",
    "VERSION:2.0",
  ...
  for (const { period, details } of schedule) {
    result = result.concat(buildEvents(calendar, period, details));
  }
...
function buildEvents(calendar, period, fields) {
  let result = [];
  ...
  for (specialDay of ["lateStart", "earlyDismissal"]) {
    calendar[specialDay].forEach((date) => {
      ...
      if (block.days.includes(getDay(date))) {
        result.push(buildEvent(date, {}, block, fields));
      }
    });
  }
...
function buildRegularEvent(calendar, { start, end, days, period }, fields) {
  let exdates = [];
